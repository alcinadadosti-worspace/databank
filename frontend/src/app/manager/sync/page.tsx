...
import { syncManagerPunches, getManagerSyncStatus, SyncStatus } from '@/lib/api';
...
export default function ManagerSync() {
  ...
  useEffect(() => {
    ...
    async function pollStatus() {
      try {
        const data = await getManagerSyncStatus(jobId!);
        setStatus(data);
  ...
  async function handleSync() {
    ...
    try {
      const data = await syncManagerPunches(manager.id, start, end);
      setJobId(data.jobId);
    } catch (err) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h2 className="text-lg font-semibold text-text-primary">Atualizar Banco de Horas</h2>
        <p className="text-sm text-text-tertiary mt-1">Sincronizar registros de ponto da sua equipe</p>
      </div>
